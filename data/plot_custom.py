...
with open(reconstruction_labels_filename, "r") as file:
    ...
    reconstruction_labels_raw = {title: np.array(vals) for title, vals in reconstruction_labels_raw.items()} 
...
# Plotting
fig, axs = plt.subplots(len(forecast_keys), 2, constrained_layout=True, figsize=(20,10))
...
for (fkey, lkey, (ax1, ax2)) in zip(forecast_keys, label_keys, axs):
    ...
    ax1.plot(reconstruction_labels["t"][-n:], reconstruction_data[fkey], label=fkey, linestyle="", marker="o", markersize=1, color="r")
    ax1.plot(reconstruction_labels["t"][-n:], reconstruction_data[lkey], label="True Signal", linestyle="-", linewidth=0.7, color="k")
    ...
    ax2.plot(times, forecast_data[fkey], label=fkey, linestyle="", marker="o", markersize=1, color="r")
    ax2.plot(times, forecast_data[lkey], label=f"True {fkey}", linestyle="-", linewidth=0.7, color="k")
