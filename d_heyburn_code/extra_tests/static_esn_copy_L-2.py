...
def normalize(data: pd.DataFrame) -> pd.DataFrame:
    return (data - data.min()) / (data.max() - data.min()) - 0.5
...
def TTS_procedure(data: np.ndarray, train_len: int, test_len: int, warmup: len, dt: int) -> tuple:
    ...
    y_train = data[warmup+dt:train_len+dt, :] # .reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    if VERBOSE:
        ...
        if NORMALIZE:
            axs[0].set_ylabel("Normalized Current", fontsize=10)
        elif STANDARDIZE1 or STANDARDIZE2:
            axs[0].set_ylabel("Standardized Current", fontsize=10)
        else:
            axs[0].set_ylabel("Current (mA)", fontsize=10)
        # Warmup current
        ...
        axs[0].text(0.05, 0.95, '(a)', horizontalalignment='right', verticalalignment='top', transform=axs[0].transAxes,
        ...
        if NORMALIZE:
            axs[1].set_ylabel("Normalized Voltage", fontsize=10)
        elif STANDARDIZE1 or STANDARDIZE2:
            axs[1].set_ylabel("Standardized Voltage", fontsize=10)
        else:
            axs[1].set_ylabel("Voltage (mV)", fontsize=10)
        # Warmup current
        ...
        axs[1].text(0.05, 0.95, '(b)', horizontalalignment='right', verticalalignment='top', transform=axs[1].transAxes,
                    fontsize=12, fontweight='normal')
    ...
    if VERBOSE:
        ...
        plt.ylabel("Node Activation", fontsize=12)
    ...
    for t in range(TEST):
        ...
        y = np.squeeze(np.asarray(np.dot(Wout, x)[1]))
        print(y)
        print(y.shape)
    ...
    if VERBOSE:
        ...
        fig, axs = plt.subplots(nrows=1, ncols=2, sharex=False, figsize=(12, 4))
        ...
        if NORMALIZE:
            axs[0].set_ylabel("Normalized Voltage", fontsize=10)
        elif STANDARDIZE1 or STANDARDIZE2:
            axs[0].set_ylabel("Standardized Voltage", fontsize=10)
        else:
        ...
        axs[0].legend(loc='upper right', fontsize=8, labels=['Forecasted', 'True'])
        axs[0].set_xlabel("Time (ms)", fontsize=10)
