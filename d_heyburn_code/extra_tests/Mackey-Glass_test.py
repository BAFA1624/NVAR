...
if __name__ == "__main__":
    ...
    WARMUP = 0
    DT = 1
    timesteps = np.linspace(0, TRAIN+TEST, TRAIN + TEST)
    ...
    data = mackey_glass(TRAIN+TEST, tau=17)

    # Plotting the train and test signals.
    if VERBOSE:
        plt.figure()
        plt.plot(timesteps[:WARMUP], data[:WARMUP], color="r", label="Warmup")
        plt.plot(timesteps[WARMUP:TRAIN], data[WARMUP:TRAIN], color="k", label="Train")
        plt.plot(timesteps[TRAIN:TRAIN + TEST], data[TRAIN:TRAIN + TEST], color='b', label="Test")
        plt.tight_layout()
        plt.show()

    # Now we perform the train/test split on the dataset, returned in the shapes we require.
    u_train = data[0:TRAIN].reshape(-1, 1)
    y_train = data[WARMUP+DT:TRAIN+DT].reshape(1, -1)
    y_test = data[TRAIN:TRAIN+TEST].reshape(-1, 1)
    ...
    # MAE:
    mae = MAE(y_true=y_test.T, y_pred=y_pred.T)
    print(f"Mean Absolute Error = {mae}")

    # MSE:
    mse = MSE(y_true=y_test.T, y_pred=y_pred.T)
    print(f"Mean Squared Error = {mse}")

    # RMSE:
    rmse = RMSE(y_true=y_test.T, y_pred=y_pred.T)
    print(f"Root Mean Squared Error = {rmse}")

    # RMSE:
    nrmse = NRMSE(y_true=y_test.T, y_pred=y_pred.T)
    print(f"Normalized (std) Root Mean Squared Error = {nrmse}")

    # R-Squared
    r2 = R2(y_true=y_test.T, y_pred=y_pred.T)
    print(f"R-squared = {r2}")

    # Maximum absolute error:
    max_err = max_error(y_true=y_test.T, y_pred=y_pred.T)
    ...
    if VERBOSE:
        ...
        if NORMALIZE:
            axs[0].set_ylabel("Prediction", fontsize=10)
        ...
        axs[0].plot(timesteps[TRAIN:TRAIN+TEST], y_pred.T, color="r", ls=None, label="Forecasted")
        axs[0].plot(timesteps[TRAIN:TRAIN+TEST], y_test, color="k", lw=1.0, label="True")
        axs[0].legend(loc='upper right', fontsize=8)
        axs[0].set_xlabel("Discrete Time", fontsize=10)
        ...
        # Point-wise Error:
        errors = pointwise_error(y_true=y_test.T, y_pred=y_pred.T)
