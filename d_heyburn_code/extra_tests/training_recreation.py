...
def TTS_procedure(data: np.ndarray, train_len: int, test_len: int, warmup: len, dt: int) -> tuple:
    ...
    I_test = data[warmup+dt:train_len+dt, 0].reshape(-1, 1)
    V_test = data[warmup+dt:train_len+dt, 1].reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    WARMUP = 0
    DT = 1
    timesteps = np.linspace(0, 1500, TRAIN + TRAIN)
    ...
    if VERBOSE:
        ...
        axs[0].plot(time, V_pred.T, color="r", lw=1.5, label="Forecasted")
        axs[0].plot(time, V_test.T, color="k", lw=1.0, label="True")
        ...
        axs[1].plot(timesteps[:TRAIN], errors, color='k')
