...
template <Weight T, Index R, Index C>
...
template <typename T, NVAR::Index R, Index C>
std::string
...
template <Weight T, Index d, Index k, Index p, Index N>
constexpr inline Vec<T, N>
combinations_with_replacement( const ConstRefVec<T, N> v ) {
    if ( p == 0 ) {
        return Vec<T, 0>{};
    }
    ...
    auto result{
        Vec<T, def_nonlinear_size<d, k, p, nonlinear_t::poly>>::Ones()
    };
...
constexpr inline Mat<T, R, C>
ridge( const Mat<T, R, C> A, const Mat<T, R, C> y, const T alpha ) {
